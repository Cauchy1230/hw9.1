...
def send_tokens_eth(w3, sender_sk, txes):
    ...
    pending = w3.eth.get_transaction_receipt(sender_pk, "pending")

    tx_ids = []
    for i, tx in enumerate(txes):
        tx_dict = {'nonce': pending + i, 'gasPrice': w3.eth.gas_price, 'gas': w3.eth.estimate_gas(
            {'from': sender_pk, 'to': tx['receiver_pk'], 'data': b'', 'amount': tx['amount']}), 'to': tx['receiver_pk'],
                   'value': tx['amount'], 'data': b''}
        tx_s = w3.eth.account.sign_transaction(tx_dict, sender_sk)

        tx_id = w3.eth.send_raw_transaction(tx_s.rawTransaction)
        tx_ids.append(tx_id)
        tx['tx_id'] = tx_id
